...
class RegtoolsWorkflow:
    ...
    def run_docker_image_as_current_user(self, image_name_and_all_args: str, stdout=None):
        ...
        subprocess.run(f'docker run --user {self.uid}:{self.gid} -v /home/ec2-user/vep_data:/opt/vep/.vep:Z {image_name_and_all_args}',
    ...
    def download_from_s3_and_run_vep(self):
        ...
        subprocess.run(
            f'aws s3 cp vep_data/{self.sample}_master.vep.vcf {self.s3_archive_upload_url}/VEP_vcfs/',
            shell=True,
            check=True)
